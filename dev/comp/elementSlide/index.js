...
define(function(require, exports, module){
  ...
  function SuperTouch(ev){
    ...
    this.results.deltaX= this.results.pageX- this.pam.startX;
    this.results.deltaY= this.results.pageY- this.pam.startY;
   ...
   // console.log(this.results.deltaY);
    ...
    if(ev.type== 'touchmove'){
      //末端滑行速率,手指停止滑动时的速度
      this.results.rateEndX= this.results.pageX- this.pam.currentX;
      this.results.rateEndY= this.results.pageY- this.pam.currentY;
    ...
    if(ev.type== 'touchend'){
      ...
      console.log(77, this.results.deltaY);
    }
  ...
  var bufferScroll= function(ev){
    ...
    var offsetY= Math.abs(this._moveY- oY);    //计算本次实际滑行总距离
    var offsetX= Math.abs(this._moveX- oX);

    //console.log(ev.rateY);
    ...
    eoY= offsetY*Math.abs(ev.rateEndY)*0.1;   //利用滑行速率计算出‘追加滑行’距离
    console.log(ev.rateEndY,offsetY,  eoY);
    eoX= offsetX*Math.abs(ev.rateX)*this.slideExtraDistanceFactor;   //利用滑行速率计算出‘追加滑行’距离*/
